...
origin = pd.read_excel('allmed.xlsx')
stop_words = stopwords.words('english') + ["'d", "'s", "'ve", "'ll", "'re", "'m", "n't"]
additionpunc = '—’“”;'
...
sent = pd.DataFrame({'para': parano, 'sentno': sentno,
                    'sent': sentences})
...
# model = Word2Vec(sentences=sententoken, window=3, sg=1,
#                 vector_size=250, epochs=150, min_count=2)
# model.save('w2vSGmed.model')
